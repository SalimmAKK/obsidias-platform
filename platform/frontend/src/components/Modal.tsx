...
export function Modal({ isOpen, onClose, title, children, maxWidth = "max-w-[680px]" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-6 border-b border-[var(--rule)] shrink-0">
          <h2 className="font-sans font-semibold text-[20px] text-[var(--ink)] m-0 leading-none">{title}</h2>
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-[10px] text-[var(--ink3)] hover:text-[var(--ink)] hover:bg-[var(--bg)] transition-colors"
